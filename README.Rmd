...
[`leaflet` package](https://rstudio.github.io/leaflet/), that loads several 
basemaps of public institutions of Spain, and the ability of downloading and 
...
```{r static, message=FALSE, warning=FALSE, dev='svg'}
library(mapSpain)
library(sf)
...
```{r tile, warning=FALSE, fig.asp=0.7, dev='svg'}
library(mapSpain)
library(sf)
...
When this option is set, `mapSpain` would look for the cached file and it 
...
> Atlas Nacional de España (ANE) [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/deed.en) 
