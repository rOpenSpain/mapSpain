...
#> ℹ Cache directory: /tmp/RtmpSPVTPw/mapSpain.

# Set an example cache.
ex <- file.path(tempdir(), "example", "cachenew")
esp_set_cache_dir(ex)
#> ℹ mapSpain cache directory is /tmp/RtmpSPVTPw/example/cachenew.
#> ℹ To save `cache_dir` for future sessions, run this function with `install` set to `TRUE`.

esp_detect_cache_dir()
#> ℹ Cache directory: /tmp/RtmpSPVTPw/example/cachenew.
#> [1] "/tmp/RtmpSPVTPw/example/cachenew"

# Restore the initial cache.
esp_set_cache_dir(my_cache)
#> ℹ mapSpain cache directory is /tmp/RtmpSPVTPw/mapSpain.
#> ℹ To save `cache_dir` for future sessions, run this function with `install` set to `TRUE`.
