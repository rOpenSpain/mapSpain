...
can be represented on static maps via `mapSpain::esp_getTiles()` or on a 
...
Galician, and also converts those names to different coding standards, as 
NUTS, ISO2 or the coding system used by the INE, that is the official 
statistic agency of Spain.
...
downloading some files, and both packages are well synchronized, so if 
you already use `giscoR` and you have set your caching options for that package 
it would be recognized too by `mapSpain`.
...
Let's analyse the distribution of women in each autonomous community with 
`tmap`:
...
[`tmap`](https://mtennekes.github.io/tmap/) package, however any package that 
