...
An example of how you can include several tiles to create a static map. We focus here on the eartquakes of the last 365 days in the island of Tenerife. Note that the tile is actively maintained so the final plot based on a static file would vary depending on the date.
...
ADIF.opaque <-
  esp_getTiles(Madrid, "RedTransporte.Ferroviario", transparent = FALSE)
...
ADIF.alpha <- esp_getTiles(Madrid, "RedTransporte.Ferroviario")
...
m2 <- leaflet(Madrid.leaflet) %>%
  addProviderEspTiles("IGNBase.Gris") %>%
  addPolygons(color = NA,
              fillColor = "red",
              group = "Polygon") %>%
  addProviderEspTiles("RedTransporte.Ferroviario", group = "Railway") %>%
  addLayersControl(overlayGroups = c("Polygon", "Railway"))
