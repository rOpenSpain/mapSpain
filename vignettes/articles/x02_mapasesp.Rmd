...
tile <- esp_get_munic(munic = "Ferrol", epsg = 3857) %>%
  esp_getTiles("MDT.Relieve", bbox_expand = 0.5, zoommin = 1)
...
lgn <- esp_get_munic_siane(munic = "^Logroño$", epsg = 3857)

lgn_mask <- esp_getTiles(lgn, "IDErioja",
  mask = TRUE, crop = TRUE,
  zoommin = 1
)
...
ggplot() +
  geom_spatraster_rgb(data = lgn_mask, maxcell = Inf) +
  theme_void() +
  labs(
    title = "Mapa Base de Logroño",
