...
georeferenced rasters or as dynamic layers in leaflet maps.

The package also includes helpers to translate and convert Autonomous Community
and province names and codes. These helpers make it easier to join, clean and
...
Most functions return `sf` objects from the **sf** package or `SpatRaster`
...
- `esp_dict_region_code()` converts text labels into Autonomous Community and
  province codes. Supported coding standards are ISO2, NUTS and INE codes
  (`codauto` and `cpro`).
...
of functions: `esp_get_xxxx()` for GISCO data and `esp_get_xxxx_siane()` for IGN
data.
