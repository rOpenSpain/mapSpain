...
```{r static3}
lgn_borders <- esp_get_munic(munic = "Logroño", epsg = 3857)

basemap <- esp_getTiles(lgn_borders, "IGNBase.Todo", bbox_expand = 0.1)

masked <- esp_getTiles(lgn_borders, "PNOA", mask = TRUE)

ggplot() +
  geom_spatraster_rgb(data = basemap, maxcell = 10e6) +
  geom_spatraster_rgb(data = masked, maxcell = 10e6, alpha = 0.6)
```
