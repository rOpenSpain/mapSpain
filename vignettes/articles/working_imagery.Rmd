...
tm_shape(BG, raster.downsample = FALSE) +
  tm_rgb() +
  tm_shape(UA.opaque, raster.downsample = FALSE) +
  tm_rgb()
```
...
tm_shape(BG, raster.downsample = FALSE) +
  tm_rgb() +
  tm_shape(UA.alpha, raster.downsample = FALSE) +
  tm_rgb()
```
...
providers you can use on functions described above. This data frame includes 
all the parameters needed to replicate the API request. In this summary only 
the parameter `url_static` is shown:
