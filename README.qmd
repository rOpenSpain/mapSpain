...
boundaries of Spain at several levels, including Autonomous Communities and
Cities, provinces, municipalities and NUTS, as `sf` objects.

**mapSpain** also includes tools to download and process static map tiles and
a [**leaflet**](https://rstudio.github.io/leaflet/) plugin for Spanish public
...
```{r}
#| label: static
#| fig-cap: Percentage of women by Autonomous Communities and Cities in Spain (2025)
library(mapSpain)
library(sf)
library(dplyr)
...
```{r}
#| label: tile
#| fig-cap: Percentage of women in Segovia by municipality (2025)
# Get census data.
census <- mapSpain::pobmun25 |>
...
create a local cache directory with `esp_set_cache_dir()`:
...
When a cache directory is set, **mapSpain** looks for cached files before
...
This package uses data from CartoBase ANE, provided by the Instituto Geográfico
Nacional.
...
[GISCO](https://ec.europa.eu/eurostat/web/gisco), Eurostat's geospatial open
data repository, which provides datasets at several resolution levels.
