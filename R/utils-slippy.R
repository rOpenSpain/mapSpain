...
# Internal functions developed by dieghernan.
bbox_tile_query <- function(bbox, zoom_levels = 0:20) {
  ...
  # Compute tile coordinates for minimum extents.
  ...
  # Compute tile coordinates for maximum extents.
...
bbox_to_tile_grid <- function(bbox, zoom = NULL, max_tiles = NULL) {
  ...
  # Build the tile grid for the requested zoom level.
...
tile_bbox <- function(x, y, zoom) {
  ...
  # Use the Web Mercator Earth radius.
  earth_radius <- 6378137

  x_min_merc <- earth_radius * lon_rad_min
  x_max_merc <- earth_radius * lon_rad_max

  # Convert latitude bounds to Web Mercator coordinates.
