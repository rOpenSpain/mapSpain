...
test_that("Test 404", {
  ...
  expect_message(n <- esp_get_hypsobath(update_cache = TRUE), "HTTP error")
  expect_null(n)
...
test_that("Cache vs non-cached", {
  ...
  # vs cache TRUE
  expect_silent(db_cached <- esp_get_hypsobath(cache = TRUE, cache_dir = cdir))
  ...
  expect_identical(
    ...
    c("siane/se89_3_orog_hipso_a_x.gpkg", "siane/se89_3_orog_hipso_a_y.gpkg")
  )
