...
The returned objects are `sf` objects from the **sf** package or
...
**mapSpain** is an API package that uses web resources. By default,
downloaded files are stored in
[`tempdir()`](https://rdrr.io/r/base/tempfile.html) for reuse during the
current session.
...
families of functions: `esp_get_xxxx()` (GISCO source) and
`esp_get_xxxx_siane()` (IGN source).
...
Imagery layers can be used as backgrounds in interactive maps and static
maps.
